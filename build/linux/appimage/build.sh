...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|spud-dev-ai|binaries|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun

  # remove check so build in docker can succeed
  sed -i 's/grep docker/# grep docker/' pkg2appimage.AppDir/usr/share/pkg2appimage/functions.sh

  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's|@@NAME@@|Spud-Insiders|g' recipe.yml
    sed -i 's|@@APPNAME@@|spud-insiders|g' recipe.yml
    sed -i 's|@@ICON@@|spud-insiders|g' recipe.yml
  else
    sed -i 's|@@NAME@@|Spud|g' recipe.yml
    sed -i 's|@@APPNAME@@|spud|g' recipe.yml
    sed -i 's|@@ICON@@|spud-editor|g' recipe.yml
  fi
